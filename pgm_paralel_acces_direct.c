...
int main(int argc, char ** argv)
{
    ...
    // Declararea ferestrelor pentru RMA
    MPI_Win row_up_win, row_down_win;
    ...
    */
    ...
    // Vom crea cele doua ferestre in care vom adauga prima linie si ultima linie de date din bucata din matricea de pixeli care ii revine procesului respectiv
    MPI_Win_create(&pold[1][1],NP*sizeof(float),1,MPI_INFO_NULL,MPI_COMM_WORLD,&row_up_win);
    MPI_Win_create(&pold[MP][1],NP*sizeof(float),1,MPI_INFO_NULL,MPI_COMM_WORLD,&row_down_win);
                            ...
                            pentru ca am considerat ca incepem de la 0 cu indexarea*/
    {
        ...
        for (int iter=0;iter<niter;iter++)
        {
            ...
            /*Folosim primul MPI_Win_fence pentru a nu lasa alte procese care sunt in spatele unuia sa preia date intr-un moment nepotrivit, 
              adica sa nu acceseze memoria mai multe procese in acelasi timp.
             *Folosim al doilea MPI_Win_fence pentru a ridica pauza pusa mai devreme.
             *Cu MPI_Get preluam datele din fereastra row_down_win in prima linie de halo, incepand cu pixelul pold[0][1] */
            MPI_Win_fence(0, row_down_win);
            MPI_Get(&pold[0][1],NP,MPI_FLOAT,recept_rank,0,NP,MPI_FLOAT,row_down_win);
            MPI_Win_fence(0, row_down_win);
            ...
            /*Folosim primul MPI_Win_fence pentru a nu lasa alte procese care sunt in spatele unuia sa preia date intr-un moment nepotrivit, 
              adica sa nu acceseze memoria mai multe procese in acelasi timp.
             *Folosim al doilea MPI_Win_fence pentru a ridica pauza pusa mai devreme.
             *Cu MPI_Get preluam datele din fereastra row_up_win in ultima linie de halo, incepand cu pixelul pold[MP+1][1] */
            MPI_Win_fence(0, row_up_win);
            MPI_Get(&pold[MP+1][1],NP,MPI_FLOAT,recept_rank,0,NP,MPI_FLOAT,row_up_win);
            MPI_Win_fence(0, row_up_win);
        ...
        if (rank == 0)
        {
            ...
            snprintf(pgm_name_out, 128, "image_640x480_WINRMA_paralel_%d.pgm", niter); /*adaug stringul cu numele fisierului de output in pgm_name_out,    
    ...
    //Eliberam ferestrele RMA create
    MPI_Win_free(&row_up_win);
    MPI_Win_free(&row_down_win);
